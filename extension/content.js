...
async function handleSkipClick() {
    if (skipButtonAppearanceTime) {
        ...
        // Store the latest time for popup display
        chrome.storage.local.set({ latestReactionTime: reactionTime });
...
function checkForUserData() {
    ...
    if (userDataDiv) {
        const newUserId = userDataDiv.getAttribute('data-user-id');
        const newUserEmail = userDataDiv.getAttribute('data-email');
        const newUserName = userDataDiv.getAttribute('data-display-name'); // Add this to page.tsx's data div
        ...
        if (newUserId !== currentUserId || newUserEmail !== currentUserEmail) {
            ...
            chrome.storage.local.set({
                userId: currentUserId,
                userEmail: currentUserEmail,
                userName: newUserName || newUserEmail // Fall back to email if no display name
            }, () => {
                if (!newUserId) {
                    // Clear storage when user is logged out
                    chrome.storage.local.remove(['latestReactionTime']);
                }
            });
